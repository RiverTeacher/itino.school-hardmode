	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		game.onenterframe = function () {
			...
			if (state == 3) {							//状態３（point６以上から）
				zoyamaImg.x += 30;
				zoyamaImg.y = 80 + Math.sin(zoyamaImg.x / 70) * 100; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			if (state == 4) {							//状態４（point９以上から）　4は初期セット状態（state=4）と移動状態（state=4.1)の2つに状態をわける		
				zoyamaImg.y = Math.random() * 200;			//ｙ座標の位置をランダムに決定
				state = 4.1;
			}
			if (state == 4.1) {							//状態４．１ 移動状態
				zoyamaImg.x += 50;	
				zoyamaImg.y +=30;					//ただ移動します
			}
			if (state == 5) {							//状態５（point１２以上から）　 ｙ軸が毎フレーム毎に変化する
				zoyamaImg.x += 30;						//移動します。
				zoyamaImg.y = Math.random() * 560;			//ｙ座標の位置を枚フレーム毎にランダム決定
			}
			...
			if (zoyamaImg.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				...
				//ゲームオーバー後のテキスト表示
				gameOverText.text = "あなたが倒した市野さんは：" + point + "人❤　   　～ハードモード～";				//テキストに文字表示 
